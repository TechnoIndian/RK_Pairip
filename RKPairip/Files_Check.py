...
class FileCheck:
	...
	def download_file(H,jar_urls_and_paths):
		R=set()
		for(I,A,J)in jar_urls_and_paths:
			B=C.os.path.basename(A)
			if C.os.path.exists(A):
				K=H.calculate_checksum(A)
				if K==J:continue
			...
			try:
				print(f"\n{C.lb}[ {C.pr}Downloading {C.lb}] {C.c}{B}",end='',flush=True);D=requests.get(I,stream=True,timeout=10)
				if D.status_code==200:
					E=int(D.headers.get('content-length',0));L=1024;F=0
					with open(A,'wb')as M:
						for G in D.iter_content(L):F+=len(G);M.write(G);N=F/E*100 if E>0 else 0;O=F/(1024*1024);P=E/(1024*1024)if E>0 else 0;Q=f"\r{C.lb}[ {C.pr}Downloading {C.lb}] {C.c}{B} {C.g}➸❥ {N:.2f}% ({O:.2f}/{P:.2f} MB)";print(Q,end='\r')
